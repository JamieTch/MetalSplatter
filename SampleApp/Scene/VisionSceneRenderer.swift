...
class VisionSceneRenderer {
    ...
    func load(_ model: ModelIdentifier?) async throws {
        ...
        if let model,
           ...
           let calibration = calibrationStore.loadCalibration(forKey: model.calibrationKey) {
    ...
    private func processCalibrationCommandsIfNeeded() {
        ...
        for command in commands {
            ...
            case .confirm:
                ...
                if case let .gaussianSplat(identifier) = model {
                    ...
                    do {
                        try calibrationStore.saveCalibration(calibration, forKey: identifier.calibrationKey)
                    } catch {
                        Self.log.error("Failed to save calibration for \(identifier.calibrationKey): \(error.localizedDescription)")
                    }
                }
                isCalibrating = false
                calibrationSnapshot = nil
