            ...
            case .confirm:
                ...
                if let model,
                   case .gaussianSplat = model {
                    ...
                    do {
                        try calibrationStore.saveCalibration(calibration, forKey: model.calibrationKey)
                    } catch {
                        Self.log.error("Failed to save calibration for \(model.calibrationKey): \(error.localizedDescription)")
                    }
                }
                isCalibrating = false
                calibrationSnapshot = nil
